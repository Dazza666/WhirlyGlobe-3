...
//	LocationInfo (name: "Moscow", lat: 55.75, lon: 37.616667),
	LocationInfo (name: "London", lat: 54.507222, lon: -10.1275),
...
class ModelsTestCase: MaplyTestCase {
	...
	override func setUpWithGlobe(_ globeVC: WhirlyGlobeViewController) {
		...
		let fullPath = Bundle.main.path(forResource: "Cruiser 2012", ofType: "obj")
		if let fullPath = fullPath {
			let model = MaplyGeomModel(obj: fullPath)
			if let model = model {
				var modelInstances = [MaplyMovingGeomModelInstance]()
				let scaletMat = MaplyMatrix(scale: 10.0/6371000.0)
                //let rotMat = MaplyMatrix (yaw: 0, pitch:  Double.pi/4, roll: Double.pi/4)
				...
				for loc in locations {
					...
					mInst.center = MaplyCoordinate3dMake(loc2d.x, loc2d.y, 25000)
                    mInst.endCenter = MaplyCoordinate3dMake(loc2d.x-0.1, loc2d.y, 25000)
